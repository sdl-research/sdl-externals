...
class HashBiTable {
 ...
 private:
  std::tr1::unordered_map<T, I, H> entry2id_;
  vector<T> id2entry_;
...
class CompactHashBiTable {
 ...
 private:
  ...
  typedef std::tr1::unordered_set<I, HashFunc, HashEqual> KeyHashSet;
...
class ErasableBiTable {
 public:
  ...
  void Erase(I s) {
    ...
    typename std::tr1::unordered_map<T, I, F>::iterator it =
 ...
 private:
  std::tr1::unordered_map<T, I, F> entry2id_;
  deque<T> id2entry_;
  const T empty_entry_;
