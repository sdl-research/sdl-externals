...
class CacheLogAccumulatorData {
 public:
  ...
  ~CacheLogAccumulatorData() {
    for(typename std::tr1::unordered_map<StateId, CacheState>::iterator it = cache_.begin();
        it != cache_.end();
        ++it)
  ...
  vector<double> *GetWeights(StateId s) {
    typename std::tr1::unordered_map<StateId, CacheState>::iterator it = cache_.find(s);
    if (it != cache_.end()) {
 ...
 private:
  ...
  void GC(bool free_recent) {
    ...
    typename std::tr1::unordered_map<StateId, CacheState>::iterator it = cache_.begin();
  ...
  std::tr1::unordered_map<StateId, CacheState> cache_;  // Cache
