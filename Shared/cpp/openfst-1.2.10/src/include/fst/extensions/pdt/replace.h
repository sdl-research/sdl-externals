...
void Replace(const vector<pair<typename Arc::Label,
  ...
  std::tr1::unordered_map<Label, size_t> label2id;
  ...
  std::tr1::unordered_set<Label> non_term_set;  // Set of non-terminals to replace
  ...
  for (StateId soff = 0; !non_term_queue.empty(); soff = ofst->NumStates()) {
    ...
    for (StateIterator< Fst<Arc> > siter(*ifst);
      ...
      for (ArcIterator< Fst<Arc> > aiter(*ifst, is);
        ...
        typename std::tr1::unordered_map<Label, size_t>::const_iterator it =
  ...
  typedef std::tr1::unordered_map<pair<size_t, StateId >, size_t,
  ...
  for (StateIterator< Fst<Arc> > siter(*ofst);
    ...
    for (size_t n = 0; !aiter->Done(); aiter->Next(), ++n) {
      Arc arc = aiter->Value();
      typename std::tr1::unordered_map<Label, size_t>::const_iterator lit =
          label2id.find(arc.olabel);
      if (lit != label2id.end()) {
