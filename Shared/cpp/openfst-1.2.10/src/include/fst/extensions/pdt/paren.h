...
class PdtParenReachable {
 ...
 private:
  ...
  std::tr1::unordered_map<Label, Label> paren_id_map_;               // Paren labels -> ID
...
bool PdtParenReachable<A>::DFSearch(StateId s, StateId start) {
  ...
  for (ArcIterator<Fst<A> > aiter(fst_, s);
    ...
    typename std::tr1::unordered_map<Label, Label>::const_iterator pit
...
class PdtBalanceData {
 public:
  ...
  typedef std::tr1::unordered_set<ParenState<A>, typename ParenState<A>::Hash> OpenParenSet;

  // Maps from open paren destination state to parenthesis ID.
  typedef std::tr1::unordered_multimap<StateId, Label> OpenParenMap;

  // Maps from open paren state to source states of matching close parens
  typedef std::tr1::unordered_multimap<ParenState<A>, StateId,
...
PdtBalanceData<A> *PdtBalanceData<A>::Reverse(
  ...
  std::tr1::unordered_set<StateId> close_sources;
  ...
  for (StateId i = 0; i < num_states; i+= split_size) {
    ...
    for (typename std::tr1::unordered_set<StateId>::const_iterator it
             = close_sources.begin();
