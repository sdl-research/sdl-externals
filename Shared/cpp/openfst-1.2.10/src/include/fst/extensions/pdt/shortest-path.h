...
class PdtShortestPathData {
 ...
 private:
  ...
  typedef std::tr1::unordered_map<SearchState, SearchData, SearchStateHash> SearchMap;

  typedef std::tr1::unordered_multimap<StateId, StateId> SearchMultimap;

  // Hash map from paren spec to open paren data
  typedef std::tr1::unordered_map<ParenSpec, SearchData, ParenHash> ParenMap;
...
class PdtShortestPath {
 ...
 public:
  ...
  typedef std::tr1::unordered_multimap<ParenState<Arc>, Arc,
 ...
 private:
  ...
  SpData sp_data_;
  std::tr1::unordered_map<Label, Label> paren_id_map_;
