...
class LabelReachableData {
 public:
  ...
  std::tr1::unordered_map<L, L> *Label2Index() {
 ...
 private:
  ...
  std::tr1::unordered_map<L, L> label2index_;        // Finds index for a label.
...
class LabelReachable {
 public:
  ...
  Label Relabel(Label label) {
    if (label == 0)
      return label;
    std::tr1::unordered_map<Label, Label> &label2index = *data_->Label2Index();
  ...
  void RelabelPairs(vector<pair<Label, Label> > *pairs,
    ...
    pairs->clear();
    std::tr1::unordered_map<Label, Label> &label2index = *data_->Label2Index();
    for (typename std::tr1::unordered_map<Label, Label>::const_iterator
  ...
  // transitons.
  const std::tr1::unordered_map<Label, Label>& Label2Index() const {
 ...
 private:
  ...
  void FindIntervals(StateId ins) {
    ...
    std::tr1::unordered_map<Label, Label> &label2index = *data_->Label2Index();
    for (typename std::tr1::unordered_map<Label, StateId>::const_iterator
  ...
  std::tr1::unordered_map<Label, StateId> label2state_;  // Finds final state for a label
